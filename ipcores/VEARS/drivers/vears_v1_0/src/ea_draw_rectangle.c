...
ea_bool ea_draw_rectangle (uint32_t x_left, uint32_t y_low, uint32_t x_right, uint32_t y_high, uint32_t color)
{
  ...
  if (ea_draw_quick_line_v(x_left,y_low, y_high , color) == ea_false) {
    returnvalue = ea_false;                      //up
  }
  if (ea_draw_quick_line_h(x_left, x_right,  y_high, color) == ea_false) {
    returnvalue = ea_false;                      //right
  }
  if (ea_draw_quick_line_v(x_right, y_low ,y_high, color) == ea_false) {
    returnvalue = ea_false;                      //down
  }
  if (ea_draw_quick_line_h(x_left ,x_right , y_low, color) == ea_false) {
    returnvalue = ea_false;                      //left
  }

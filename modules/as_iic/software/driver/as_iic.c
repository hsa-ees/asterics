...
const char *as_iic_err_to_str(uint8_t error_number) {
  switch (error_number) {
    ...
    case 0x2:
      return "Timeout: IO not ready!";
    case 0x3:
      return "Timeout: Send address";
    case 0x4:
      return "Timeout: Send data";
    case 0x5:
      return "Timeout: Receive data";
    case 0x6:
      return "NACK after address!";
    case 0x7:
      return "NACK after data!";
    case 0x8:
      return "NACK after register!";
    case 0x9:
      return "IO not ready after read!";
    case 0xA:
      return "Timeout - general transmission";
    case 0xB:
      return "No acknowledgement from slave";
    case 0xC:
      return "Invalid frequency. 10kHz - 1MHz is supported!";
    case 0xD:
      return "The hardware is uninitialized! Call as_iic_init first!";
    default:
...
uint32_t as_iic_get_status(as_hardware_address_t base_addr) { return as_reg_read(base_addr + AS_IIC_STATUSCONTROL_OFFSET); }

uint32_t as_iic_get_data_rx(as_hardware_address_t base_addr) { return as_reg_read(base_addr + AS_IIC_DATA_RX_OFFSET); }

uint32_t as_iic_get_data_tx(as_hardware_address_t base_addr) { return as_reg_read(base_addr + AS_IIC_DATA_TX_OFFSET); }

/***** Set register values *****/

void as_iic_set_control(as_hardware_address_t base_addr, uint8_t control) {
  as_reg_write(base_addr + AS_IIC_STATUSCONTROL_OFFSET, (uint32_t)((control & AS_IIC_CTRL_MASK) << 16));
}

void as_iic_set_data_register(as_hardware_address_t base_addr, uint32_t data) { as_reg_write(base_addr + AS_IIC_DATA_TX_OFFSET, data); }

void as_iic_set_data_tx(as_hardware_address_t base_addr, uint8_t *byte) {
  as_reg_write(base_addr + AS_IIC_DATA_TX_OFFSET, (uint32_t)(*byte << 24) | (as_reg_read(base_addr + AS_IIC_DATA_TX_OFFSET) & AS_IIC_TX_DATA_MASK));
}

void as_iic_set_scl_div(as_hardware_address_t base_addr, uint32_t div) {
  as_reg_write(base_addr + AS_IIC_DATA_TX_OFFSET,
               (uint32_t)(div & AS_IIC_TX_DATA_MASK) | (as_reg_read(base_addr + AS_IIC_DATA_TX_OFFSET) & AS_IIC_SCL_DIV_MASK));
}

/***** Get register values *****/

uint8_t as_iic_get_rx_byte(as_hardware_address_t base_addr) { return (uint8_t)(as_reg_read(base_addr + AS_IIC_DATA_RX_OFFSET) >> 8); }

uint8_t as_iic_get_status_reg(as_hardware_address_t base_addr) { return (uint8_t)(as_reg_read(base_addr + AS_IIC_STATUSCONTROL_OFFSET) & AS_IIC_STATUS_MASK); }

uint8_t as_iic_get_control_reg(as_hardware_address_t base_addr) {
  return (uint8_t)((as_reg_read(base_addr + AS_IIC_STATUSCONTROL_OFFSET) & AS_IIC_CTRL_READ_MASK) >> 16);
}

/***** Check as_iic status bits *****/

AS_BOOL as_iic_is_ready(as_hardware_address_t base_addr) { return ((as_iic_get_status_reg(base_addr) & AS_IIC_READY) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_is_io_ready(as_hardware_address_t base_addr) { return ((as_iic_get_status_reg(base_addr) & AS_IIC_IO_READY) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_is_active(as_hardware_address_t base_addr) { return ((as_iic_get_status_reg(base_addr) & AS_IIC_BUS_ACTIVE) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_ack_received(as_hardware_address_t base_addr) { return ((as_iic_get_status_reg(base_addr) & AS_IIC_ACK_RECEIVED) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_is_stalled(as_hardware_address_t base_addr) { return ((as_iic_get_status_reg(base_addr) & AS_IIC_STALLED) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_is_waiting(as_hardware_address_t base_addr) { return ((as_iic_get_status_reg(base_addr) & AS_IIC_WAITING) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_is_readwrite(as_hardware_address_t base_addr) { return ((as_iic_get_control_reg(base_addr) & AS_IIC_RW) > 0) ? AS_TRUE : AS_FALSE; }

AS_BOOL as_iic_is_initialized(void) { return as_iic_initialized_flag ? AS_TRUE : AS_FALSE; }

/***** Set as_iic control bits *****/

void as_iic_set_transmit_start(as_hardware_address_t base_addr) {
  as_iic_set_control(base_addr, (as_iic_get_control_reg(base_addr) & (~AS_IIC_TSTART)) | AS_IIC_TSTART);
}

void as_iic_set_readwrite(as_hardware_address_t base_addr, uint8_t value) {
  if (value) {
...
void as_iic_set_transmit_stop(as_hardware_address_t base_addr) {
  as_iic_set_control(base_addr, (as_iic_get_control_reg(base_addr) & (~AS_IIC_TEND)) | AS_IIC_TEND);
}

void as_iic_set_data_ready(as_hardware_address_t base_addr) {
  as_iic_set_control(base_addr, (as_iic_get_control_reg(base_addr) & (~AS_IIC_DATA_READY)) | AS_IIC_DATA_READY);
}

void as_iic_set_ack_mod(as_hardware_address_t base_addr) {
  as_iic_set_control(base_addr, (as_iic_get_control_reg(base_addr) & (~AS_IIC_ACK_MODIFIER)) | AS_IIC_ACK_MODIFIER);
}

void as_iic_set_initialized(uint8_t value) { as_iic_initialized_flag = value; }

void as_iic_reset_hw_state(as_hardware_address_t base_addr) { as_iic_set_control(base_addr, AS_IIC_RESET); }

void as_iic_reset(as_hardware_address_t base_addr) {
...
AS_BOOL as_iic_busy_wait_for_hwready(as_hardware_address_t base_addr, uint32_t timeout_ns) {
  uint32_t counter;
...
AS_BOOL as_iic_busy_wait_for_wait(as_hardware_address_t base_addr, uint32_t timeout_ns) {
  uint32_t counter;
  for (counter = 0; counter < timeout_ns; counter += 100) {
...
AS_BOOL as_iic_busy_wait_for_ioready(as_hardware_address_t base_addr, uint32_t timeout_ns) {
  uint32_t counter;
  for (counter = 0; counter < timeout_ns; counter += 100) {
...
uint8_t as_iic_init(as_hardware_address_t base_addr, uint32_t iic_bus_freq) {
...
uint8_t as_iic_start_transaction(as_hardware_address_t base_addr, uint8_t *slave_addr, uint8_t *data, uint8_t readwrite, uint8_t modifier) {
  ...
  if (modifier & AS_IIC_MOD_ONLY_ADDRESS) {
    ...
    if (as_iic_ack_received(base_addr) == AS_FALSE && !(modifier & (AS_IIC_MOD_MASTER_ACK | AS_IIC_MOD_IGNORE_ACKNOWLEDGE))) {
...
uint8_t as_iic_stop_read_transaction(as_hardware_address_t base_addr, uint8_t *read_data) {
...
uint8_t as_iic_stop_write_transaction(as_hardware_address_t base_addr) {
  // stop the transaction.
  as_iic_set_control(base_addr, AS_IIC_STOP_TRANS);
...
uint8_t as_iic_write_transaction(as_hardware_address_t base_addr, uint8_t *byte, uint8_t modifier) {
...
uint8_t as_iic_read_transaction(as_hardware_address_t base_addr, uint8_t *read_data, uint8_t modifier) {
  // check whether the previous transaction was read or write.
  AS_BOOL prev_rw = as_iic_is_readwrite(base_addr);
...
uint8_t as_iic_get_byte(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *read_data) {
  return as_iic_get_byte_mod(base_addr, slave_addr, read_data, AS_IIC_MOD_NONE);
}

uint8_t as_iic_write_byte(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *byte) {
  return as_iic_write_byte_mod(base_addr, slave_addr, byte, AS_IIC_MOD_NONE);
}

uint8_t as_iic_get_byte_mod(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *read_data, uint8_t modifier) {
  uint8_t report;
...
uint8_t as_iic_write_byte_mod(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *byte, uint8_t modifier) {
  uint8_t report;
...
uint8_t as_iic_get_bytes(as_hardware_address_t base_addr, uint8_t slave_addr, uint16_t byte_count, uint8_t *read_data) {
  uint8_t report = 0;
  uint16_t count = 0;
  byte_count--;
...
uint8_t as_iic_write_bytes(as_hardware_address_t base_addr, uint8_t slave_addr, uint16_t byte_count, uint8_t *bytes) {
  uint8_t report = 0;
  uint16_t count = 1;
  // start a write transaction (send the address) and send the first data byte.
  report = as_iic_start_transaction(base_addr, &slave_addr, &(bytes[0]), AS_IIC_WRITE, AS_IIC_MOD_NONE);
  if (report != AS_IIC_OK) return report;
...
uint8_t as_iic_read_reg(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *reg, uint8_t *read_data) {
  uint8_t report = 0;
...
uint8_t as_iic_write_reg(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *reg, uint8_t *data) {
...
uint8_t as_iic_read_regs(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *reg, uint16_t byte_count, uint8_t *read_data) {
  uint8_t report = 0;
  uint16_t count = 0;
  byte_count--;
...
uint8_t as_iic_set_regpointer(as_hardware_address_t base_addr, uint8_t slave_addr, uint8_t *pointer) {
  uint8_t report = 0;
  report = as_iic_start_transaction(base_addr, &slave_addr, pointer, AS_IIC_WRITE, AS_IIC_MOD_MASTER_ACK);
  if (report != AS_IIC_OK) return report;
  return as_iic_stop_write_transaction(base_addr);
}

 ...
 * 
 * It is designed to be used to configure cameras to use 
 * with the ASTERICS framework, therefore many of the advanced features are
 * missing from this module, to keep it as small as possible.
 * 
 ...
 * This range is dependent on the FPGA System frequency
 * and the generic "SCL_REGISTER_WIDTH" of the module.
 * The module only supports standard and fast mode.
 * Only 7 bit addressing is supported.
 * The module has to be the only master on the IIC bus, 
 * as no arbitration method is implemented and the module does not check 
 * for activity on the bus by other IIC masters.
 * @}
 */
...
/**
 * @defgroup as_iic_regs Register Offsets
 * @{
 */
...
/**
 * @defgroup as_iic_bits Control and Status Bit Defines
 * @{
 */
...
/**
 * @defgroup as_iic_masks Register Bit Masks
 * @{
 */
...
/**
 * @defgroup as_iic_err Error Codes
 * @{
 */
...
/**
 * @defgroup as_iic_timeouts Timeout Defines
 * @{
 */

//! Timeout base, should be approx 1 µs
//! Set this value according to the processor frequency (AS_SYSTEM_CLOCK_HZ)
//! to equal to the number of cycles to amount to approximatly 1 µs
#define AS_IIC_TIMEOUT_BASE 1000

#ifdef AS_IIC_USE_VERY_LONG_TIMEOUT
//! very long timeout 1 ms
#define AS_IIC_LONG_TIMEOUT 1000  
#else
//! long timeout 100 µs
#define AS_IIC_LONG_TIMEOUT 100  
#endif

#ifdef AS_IIC_USE_VERY_LONG_TIMEOUT
//! longer short timeout 100 µs
#define AS_IIC_SHORT_TIMEOUT 100  
#else
//! short timeout 10 µs
#define AS_IIC_SHORT_TIMEOUT 10  
#endif
...
/**
 * @defgroup as_iic_debug Debug Functions
 * @{
 */
...
/**
 * @defgroup as_iic_reg_funcs Direct Register Access Functions
 * @{
 */
 ...
 *                      register.
 */
uint32_t as_iic_get_status(as_hardware_address_t base_addr);
 ...
 */
uint32_t as_iic_get_data_rx(as_hardware_address_t base_addr);
 ...
 *                      div" register.
 */
uint32_t as_iic_get_data_tx(as_hardware_address_t base_addr);
 ...
 */
void as_iic_set_data_register(as_hardware_address_t base_addr, uint32_t data);
 ...
 *                      receive" register.
 */
uint8_t as_iic_get_rx_byte(as_hardware_address_t base_addr);
 ...
 */
uint8_t as_iic_get_status_reg(as_hardware_address_t base_addr);
...
/**
 * @defgroup as_iic_bitchecks Status Bit Checking Functions
 * @{
 */
 ...
 *                      AS_BOOL.
 */
AS_BOOL as_iic_is_ready(as_hardware_address_t base_addr);
 ...
 *                      AS_BOOL.
 */
AS_BOOL as_iic_is_io_ready(as_hardware_address_t base_addr);
 ...
 * @param base_addr     Address of the corresponding as_iic
 *                      hardware module (see also as_hardware.h)
 * @return              If the "bus_active" bit is set as an AS_BOOL.
 */
AS_BOOL as_iic_is_active(as_hardware_address_t base_addr);
 ...
 *                      AS_BOOL.
 */
AS_BOOL as_iic_ack_received(as_hardware_address_t base_addr);
 ...
 *                      AS_BOOL.
 */
AS_BOOL as_iic_is_stalled(as_hardware_address_t base_addr);
 ...
 *                      AS_BOOL.
 */
AS_BOOL as_iic_is_waiting(as_hardware_address_t base_addr);
 ...
 *                      AS_BOOL.
 */
AS_BOOL as_iic_is_readwrite(as_hardware_address_t base_addr);
...
// set control bits ~~~~~~~~~~~~~~~~~~~~~
/**
 * @defgroup as_iic_bitsetters Control Bit Setting Functions
 * @{
 */
/**
 ...
 *                      hardware module (see also as_hardware.h)
 */
void as_iic_reset_hw_state(as_hardware_address_t base_addr);
...
// busy waiting ftw ~~~~~~~~~~~~~~~~
/**
 * @defgroup as_iic_wait Busy Waiting Functions
 * @{
 */
/**
...
/**
 * @defgroup as_iic_helpers Functions Implementing Partial Transactions
 * @{
 */
...
/**
 * @defgroup as_iic_user User Level Functions
 * @{
 */

 ...
 * @{
 *  @defgroup as_memio as_memio: Linux Memory Interface
 * @}
 * This software module is for use with the ASTERICS Linux Kernel driver to allow
 * communication between Linux and the ASTERICS hardware.
 */
...
#define AS_MEMIO_DEFAULT_INTERFACE_WIDTH    64U
 ...
 *
 * @param as_reader_writer_buffer_size          Configures the size of the buffer in bytes for
...
/** Manages posession of buffers */
typedef struct as_memio_buffer_handler_s {
  /** Array of pointers to buffer objects */
  as_buffer_obj_t* buffers[2];
  /** Buffers that are currently waiting for completion in the order they have been enqueued */
  as_buffer_obj_queue_t queue;
  /** Amount of pointers held by field buffers */
  size_t available_buffers;
} as_memio_buffer_handler_t;

/** Settings regarding MemReader/Writer Hardware */
typedef struct as_memio_hw_settings_s {
  /** Base Address of the memreader/writer module */
  as_hardware_address_t baseaddr;
  /** Size of a word */
  uint32_t wordsize;
  ...
  /** Module is Writer */
  AS_BOOL read_not_write;
  /** Module needs manual cache flushing/invaldiation */
  AS_BOOL manage_cache;
 ...
 *
 * This function allocates memory for each HW module buffer and initializes the as_memio
 * infrastructure for buffer management. Depending on settings of "memio_config" and
 ...
 *
 * @param config                            This structure is used to setup the buffer(s)
 ...
 *
 */
struct as_memio_file_s* as_memio_open(as_hardware_address_t as_reader_writer_baseaddr, as_memio_config_t* memio_config, uint8_t flags);

/**
 * Function: as_memio_close
 *
 * This function de-initializes the ASTERICS streaming interface and releases all acquired
 * memory for setting up its infrastructure (i.e. buffer, configuration structures).
 *
 ...
 * to the buffer which is used by the as_memreader hardware module.
 * This function may copy less data from the user buffer than requested by "count"
 * if the buffer has not enough empty entries.
 *
 ...
 *
 */
ssize_t as_memio_write(struct as_memio_file_s* fd, const void* buffer, uint32_t count);
 ...
 *
 */
ssize_t as_memio_read(struct as_memio_file_s* fd, void* buffer, uint32_t count);

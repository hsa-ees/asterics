...
uint32_t ov7670_iic_reg_set(SENS_CONF* iic_channel, uint8_t iic_adr, uint8_t reg_adr, uint8_t value);
uint32_t ov7670_iic_reg_get(SENS_CONF* iic_channel, uint8_t iic_adr, uint8_t reg_adr, uint8_t *value);
SENS_CONF* ov7670_get_iic_channel(uint32_t* base_addr);

/* Simplified usage of IIC write: */
#define IIC_SETREG(ch,r,v) ov7670_iic_reg_set((ch), OV7670_IIC_SLV_ADDR, (r),(v));
...
void ov7670_set_registers_vga_gray(uint32_t* base_addr){
    ...
    IIC_SETREG(channel, 0x13, 0x8f) /* & IIC_SETREG(0x80" OR X"40" OR X"20")), (COM8) */
    ...
    IIC_SETREG(channel, 0x13, 0xe5) /* & IIC_SETREG(0x80" OR X"40" OR X"20" OR X"04"OR X"01")), (COM8) */ 
    ...
    IIC_SETREG(channel, 0x13, 0xe5) /*& IIC_SETREG(0x80" OR X"40" OR X"20" OR X"04" OR X"01")), */ /* COM8 AGC AEC */
                              ...
                              uint32_t* iic_base_addr)
{
    ...
    IIC_SETREG(channel, COM2_REG, 0x00) /* Set Video Output Strength to 1x (lowest possible value to avoid crosstalk) */

    // Check for sensor ID:
    if (!as_sensor_ov7670_identify(base_addr)) return 0;
...
uint32_t ov7670_iic_reg_set(SENS_CONF* iic_channel, uint8_t iic_adr, uint8_t reg_adr, uint8_t value)
{
    ...
    xil_printf("Set: Send to [0x%02X]: [0x%02X] <- [0x%02X] \n\r", iic_adr, reg_adr, value);
      ...
      if(iic_channel->iic_type == OV7670_AS_IIC){
          send = as_iic_write_reg(iic_channel->iic_base_addr, iic_adr, &reg_adr, &value);
      #ifdef DEBUG_PRINT
          xil_printf("Got from as_iic: %x\n", send);
      #else
          if(send != 0) xil_printf("Got from as_iic: %x\n\r", send);
      #endif
      }
    #endif
    #ifdef AS_USING_XILINX_PL_IIC
      if(iic_channel->iic_type == OV7670_XILINX_PL_IIC){
          send = XIic_Send(iic_channel->iic_base_addr, iic_adr>>1, data, 2, XIIC_STOP);
      }
    #endif
    #ifdef AS_USING_XILINX_PS_IIC
...
uint32_t ov7670_iic_reg_get(SENS_CONF* iic_channel, uint8_t iic_adr, uint8_t reg_adr, uint8_t *value)
{
    ...
    #ifdef AS_USING_AS_IIC
      if(iic_channel->iic_type == OV7670_AS_IIC){
          send = as_iic_read_reg(iic_channel->iic_base_addr, iic_adr, &reg_adr, value);
      #ifdef DEBUG_PRINT
          xil_printf("Got from as_iic: %x\n\r", send);
      #else
          if(send != 0) xil_printf("Got from as_iic: %x\n\r", send);

...
#if AS_OS_POSIX
static int as_control_fd;
static int as_iic_fd;
static int as_regio_fd;
#endif /* AS_OS_POSIX */
...
struct as_tasklet_s *as_tasklet_new(as_tasklet_f *func, void *data)
{
  ...
  AS_ASSERT(tasklets_top < MAX_TASKLETS);
...
void as_tasklet_del(struct as_tasklet_s *tasklet)
{
  ...
  n = tasklet - tasklets;
  AS_ASSERT(n >= 0 && n < tasklets_top);
  if (n < tasklets_top - 1)
    tasklets[n] = tasklets[tasklets_top - 1];
  tasklets_top--;
}
...
void as_sleep(int nanoseconds)
{
  ...
  while (nanoseconds > 0)
  {
    ...
    if (i >= 1000000)
    { /* one 'as_yield' call approx. every millisecond */
      as_yield();
      i -= 1000000;
    }
  }
...
struct as_thread_s *as_thread_start(as_thread_func_f *func, void *data)
{
  ...
  AS_ASSERT(pthread_create(&thread->pthread, NULL, func, data) == 0);
  return thread;
}
...
void *as_thread_join(struct as_thread_s *thread)
{
  ...
  AS_ASSERT(pthread_join(thread->pthread, &ret) == 0);
  free(thread);
  return ret;
}
...
struct as_mutex_s *as_mutex_new()
{
  ...
  mutex = as_malloc(sizeof(struct as_mutex_s));
  mutex_init(&mutex->lmutex);
  return mutex;
}

void as_mutex_del(struct as_mutex_s *mutex)
{
  mutex_destroy(&mutex->lmutex);
  as_free(mutex);
}

void as_mutex_lock(struct as_mutex_s *mutex)
{
  mutex_lock(&mutex->lmutex);
}

AS_BOOL as_mutex_trylock(struct as_mutex_s *mutex)
{
  return mutex_trylock(&mutex->lmutex) == 0;
}

void as_mutex_unlock(struct as_mutex_s *mutex)
{
  mutex_unlock(&mutex->lmutex);
}

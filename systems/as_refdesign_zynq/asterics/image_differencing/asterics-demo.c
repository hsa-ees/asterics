...
--                 To be used alongside the as_refdesign_zynq image differencing
demo.
----------------------------------------------------------------------------------
...
int main() {
  ...
  AS_ASSERT(delay_buf != NULL);
  AS_ASSERT(diff_image != NULL);

  // Init VEARS ...
  XGpio_DiscreteWrite(&gpio_leds, 1, 0x01); // Set LED0: Starting VEARS
  printf("VEARS: img=0x%08x (@%luK)\n", (uint32_t)diff_image,
         ((uint32_t)diff_image) / 1024);
  vears_init(VEARS_BASEADDR, delay_buf);
  vears_overlay_on(
      VEARS_BASEADDR); /* We may move this down to hide the drawing process */

  // Init ASTERICS ...
  XGpio_DiscreteWrite(&gpio_leds, 1,
                      0x03); // Set LED1: VEARS started, starting ASTERICS
  printf("ASTERICS:\n");
  ...
  as_reader_writer_set_section_addr(AS_MODULE_BASEADDR_WRITER1,
                                    (uint32_t *)delay_buf);
  as_reader_writer_set_section_size(AS_MODULE_BASEADDR_WRITER1,
                                    FRAME_SIZE * sizeof(uint8_t));

  //   4) MemReader #0 ...
  printf("   - as_reader_writer #3 (reader0)\n");
  as_reader_writer_init(AS_MODULE_BASEADDR_AS_MEMREADER_0, NULL);
  as_reader_writer_set_section_addr(AS_MODULE_BASEADDR_AS_MEMREADER_0,
                                    (uint32_t *)delay_buf);
  as_reader_writer_set_section_size(AS_MODULE_BASEADDR_AS_MEMREADER_0,
                                    FRAME_SIZE * sizeof(uint8_t));

  XGpio_DiscreteWrite(&gpio_leds, 1,
                      0x07); // Set LED2: ASTERICS started, starting main loop

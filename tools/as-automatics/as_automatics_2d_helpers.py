...
def data_widths_to_window(columns: Port.DataWidth, rows: Port.DataWidth) -> WindowDef:
    if columns.sep == "to":
        from_x = columns.a
        to_x = columns.b
    else:
...
def parse_gwindow_type(width_str: str, generics: list) -> list:
    ...
    for par in parens:
        ...
        value0 = eval_vhdl_expr(values[0].strip().upper(), "data width", gendict)
        value1 = eval_vhdl_expr(values[1].strip().upper(), "data width", gendict)

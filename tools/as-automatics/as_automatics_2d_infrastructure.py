...
class WindowRef:
    ...
    def update(self, ref):
        if isinstance(ref, WindowRef):
            ...
            raise TypeError("WindowRef.update() doesn't accept type {}!"
                            .format(type(ref)))
        self.__update__()
    ...
    def __update__(self):
        ...
        if (self.refnum is None
                and self.col is not None and self.row is not None):
            self.refnum = self.columns * self.row + self.col
        else:
            self.row = int(self.refnum / self.columns)
            if self.row > self.rows:
                raise AttributeError("Reference '{}' out of bounds!"
                                    .format(repr(self)))
    ...
    def __comp_type_error__(other):
        raise TypeError("Comparison of WindowRef with '{}' not allowed!"
                        .format(type(other)))

    @staticmethod
    def __math_type_error__(other):
        raise TypeError("Arithmatic of WindowRef with '{}' not allowed!"
                        .format(type(other)))
...
class AsConnectionRef(WindowRef):
    ...
    def __repr__(self):
        return "{} {} {}".format(self.refnum, self.port.direction,
                                 self.port.code_name)
...
class AsLayer():
    ...
    def __gt__(self, other):
        if not isinstance(other, AsLayer):
            raise TypeError("Comparison between AsLayer and {} unsupported!"
                            .format(type(other)))
        ...
        if any((True for sec in self.sections if sec in section)):
            LOG.debug(
                ("Tried adding section '%s' to layer '%s'. Section alread"
                 "y present or overlapping!"), str(section), self.name)
            return False
        self.sections.append(section)
        return True

    def is_valid_ref(self, ref: WindowRef, direction: str) -> bool:
        if any((self.input is None, self.start_ref is None,
                self.end_ref is None)):
    ...
    def remove_section(self, section) -> bool:
        try:
            ...
            LOG.debug("Couldn't remove '%s' from '%s': '%s'",
                      repr(section), self.name, str(err))
            return False
        return True
    ...
    def set_input(self, input_ref: AsConnectionRef):
        ...
        if is_data_width_resolved(port.data_width):
            ...
            if self.data_width != port_data_width:
                LOG.error(("Incompatiple layer input detected! '%s'(%s) -> "
                           "'%s'(%s)"), port.code_name, port_data_width,
                          self.name, self.data_width)
                raise AsConnectionError(
                        "Layer and input port data widths differ!")
        if ((input_ref.row < self.parent.rows) and
                (input_ref.col < self.parent.columns) and
                (input_ref.row >= 0 and input_ref.col >= 0)):
            self.input = input_ref
            LOG.debug("Set input for layer '%s' to %s.",
                      self.name, str(input_ref))
        else:
    ...
    def add_output(self, output_ref: AsConnectionRef,
                   ignore_offset: bool = False) -> AsLayerRef:
        ...
        if found:
            LOG.debug("Did not add output at '%s', already in layer '%s'!",
                      str(to_add), self.name)
        else:
            if ((to_add.row < self.parent.rows) and
                    (to_add.col < self.parent.columns)):
                self.output_refs.append(to_add)
                LOG.debug("Added filter output %s to layer '%s'.",
                          str(to_add), self.name)
            else:
    ...
    def set_bounds(self, ref_from: WindowRef, ref_to: WindowRef):
        ...
        if ((ref_from.col >= self.parent.columns) or
                (ref_to.col >= self.parent.columns)):
            # TODO: Error handling
            raise AttributeError("Out of bounds!")
...
class AsWindowSection():
    ...
    def __str__(self) -> str:
        return "{} -> {} | {}".format(self.start_ref, self.end_ref,
                                      self.impl_type)
    ...
    def __contains__(self, other):
        ...
        if ((other.start_ref >= self.start_ref) and
                (other.end_ref <= self.start_ref)):
            return True
        return False
    ...
    def overlaps(self, other) -> bool:
        ...
        if ((other.end_ref > self.start_ref) and
                (other.start_ref < self.end_ref)):
            return True
        return False

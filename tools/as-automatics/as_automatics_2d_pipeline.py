...
from as_automatics_exceptions import AsAssignError, AsModuleError, AsConnectionError
...
class As2DWindowPipeline:
    ...
    UserConJob = namedtuple("UserConnectionJob", ["type", "inter", "layer", "anchor"])
    ...
    def set_bram_threshold(
        self, threshold_in_bits: int = -1, threshold_in_bytes: int = -1
    ):
    ...
    def add_layer(self, name: str, data_width: int, baselayer: bool = False) -> AsLayer:
        ...
        if name in [l.name for l in self.layers]:
            raise AttributeError("Layer with name {} already exists!".format(name))
        ...
        layer.set_bounds(
            WindowRef(0, 0), WindowRef(col=self.columns - 1, row=self.rows - 1)
        )
            ...
            for idx in range(len(outputs)):
                ...
                if idx == 0:
                    ...
                    if outref.refnum != sec.start_ref.refnum:
                        to_split.append(WindowRef.get_ref_from_refnum(outref.refnum))
                    continue
    ...
    def __find_brams__(self):
        ...
        for idx in range(len(over) - 1):
            ...
            for csec in over[idx + 1 :]:  # Compare against all larger sections
                ...
                if sec in csec:  # If the larger section contains this section
                    ...
                    LOG.debug(
                        "Marking section '%s' of %s to be removed!",
                        repr(sec),
                        repr(sec.layers),
                    )
                    break
        ...
        for sec in over:
            LOG.debug("Adding section '%s' of %s...", repr(sec), repr(sec.layers))
    ...
    def __filter_unsuitable__(self, sections: list, threshold: int = 0) -> list:
    ...
    def add_module(
        self, entity_name: str, user_name: str = "", repository: str = ""
    ) -> AsWindowModule:
        ...
        if not user_name:
            count = len(
                [1 for mod in self.window_modules if mod.entity_name == entity_name]
            )
    ...
    def get_layer(self, layer_name: str):
        ...
        return next((layer for layer in self.layers if layer.name == layer_name), None)
    ...
    def connect(self, interface: Interface, layer: AsLayer, offset: tuple = None):
    ...
    def __run_user_connect_calls__(self):
        ...
        if not base:  # Need at least one base layer...
            ...
            raise AsConnectionError(
                ("Could not start connection process, " "no base layer specified!")
            )
        LOG.debug("Pipeline: Base layers %s:", str(base))

        # Sort user connection jobs
        input_cons = [
            con for con in self.user_connections if con.type is self.LAYER_INPUT
        ]
        output_cons = [
            con for con in self.user_connections if con.type is self.LAYER_OUTPUT
        ]
        LOG.debug(
        ...
        for layer in base:
            LOG.debug("Pipeline: Starting job handler for layer '%s'...", layer.name)
            self.__rec_conjob_handler__(layer, input_cons, output_cons, done_jobs)
    ...
    def __rec_conjob_handler__(
        self, layer: AsLayer, input_conjobs: list, output_conjobs: list, complete: list
    ):
        ...
        for conjob in [con for con in input_conjobs if con.layer is layer]:
            if conjob not in complete:  # Run if not handled
                LOG.debug(
                    "UserCon: Setting input of layer '%s' from '%s'...",
                    layer.name,
                    repr(conjob.inter),
                )
                self.__connect_layer_input__(conjob.inter, conjob.layer, conjob.anchor)
        ...
        for conjob in [con for con in output_conjobs if con.layer is layer]:
            if conjob not in complete:  # Run if not handled
                LOG.debug(
                    "UserCon: Connecting '%s' to '%s'...",
                    repr(conjob.layer),
                    repr(conjob.inter),
                )

                self.__connect_layer_output__(conjob.inter, conjob.layer, conjob.anchor)
                complete.append(conjob)
                interface = conjob.inter
                # Handle all jobs with the module, that takes data from
                # the current layer -> Recursion! We're going down!
                for conjob in [
                    con for con in input_conjobs if con.inter.parent is interface.parent
                ]:
                    LOG.debug(
                        "UserCon: Running handler for layer '%s'", repr(conjob.layer)
                    )
                    self.__rec_conjob_handler__(
                        conjob.layer, input_conjobs, output_conjobs, complete
                    )

    def __get_base_layers__(self) -> list:
        return [layer for layer in self.layers if layer.base_layer]

    def __connect_layer_input__(
        self, inter: Interface, layer: AsLayer, anchor: WindowRef
    ):
        ...
        if window:
            if len(inter.references) > 1:
                ...
                raise AsConnectionError(
                    ("Layer input interfaces must have only" " one port!")
                )
            else:
            ...
            try:  # Assuming AsStream interface!
                ...
                LOG.error(
                    "Could not find port 'data' in '%s'! - '%s'", inter.name, str(err)
                )
                raise AsConnectionError(
                    (
        ...
        if window:
            ...
            raise TypeError(
                "Unkown module type for connection: {}".format(type(module))
            )

    def __connect_layer_output__(
        self, inter: Interface, layer: AsLayer, anchor: WindowRef
    ):
        ...
        if layer.input is None:  # Just making sure!
            ...
            raise AsConnectionError(
                ("Cannot connect outputs to layer " "without an input!")
            )

...
if __name__ == "__main__":
    ...
    class GUI(qtw.QMainWindow):
        ...
        def __init__(self, auto: AsAutomatics):
            ...
            self.modlist.setHorizontalHeaderLabels(("Module Name", "Type", "Repository"))
            ...
            self.interlist.setHorizontalHeaderLabels(("Name", "Type", "Direction / Value"))
        ...
        def module_clicked(self):
            ...
            if ((self.crt_module is None) or 
                ...
                if module:
                    ...
                    self.status.setText("Selected module: {}"
                                        .format(module.entity_name))
                    self.infobox.setText("{} from repository {}"
                            .format(module, modrepo))
                else:
        ...
        def inter_clicked(self):
            ...
            intername, intertype, direction = \
                self.get_selected_item(self.interlist)
            if intertype in ("Port", "StandardPort"):
                ...
                if isinstance(self.crt_interface, SlaveRegisterInterface):
                    inter = self.crt_module.get_slave_register_interface(
                            intername)
                inter = self.crt_module.get_interface(intername, direction,
                                                      intertype)
                if inter:
                    self.crt_interface = inter
                    self.crt_port = None
                    self.crt_generic = None
                    select_text = "Selected interface '{}' of module '{}'" \
                            .format(intername, self.crt_module.entity_name)
                    self.status.setText(select_text)
                    print(str(inter))
                    self.infobox.setText(
                        "Interface '{}' ({}) with direction '{}' of:\n{}" \
                            .format(intername, intertype, direction,
                                    str(self.crt_module)))
                else:
        ...
        def port_clicked(self, from_interlist: tuple = None):
            if from_interlist:
                ...
                portname, porttype, direction = \
                    self.get_selected_item(self.portlist)
                port = self.crt_interface.get_port(portname,
                                                   suppress_error=True)
            if port:
                self.crt_port = port
                self.crt_generic = None
                ...
                if not from_interlist:
                    select_text = ("{} '{}' ({}) with direction '{}' of "
                                "interface '{}' of module '{}'").format(
                                porttype, portname, port.code_name, direction, 
                                self.crt_interface.name,
                                self.crt_module.entity_name)
                else:
                    select_text = ("Port '{}' ({}) with direction '{}' of "
                                   "module '{}'").format(portname,
                                   port.code_name, direction,
                                   self.crt_module.entity_name)
                self.status.setText(select_text)
                print(str(port))
                self.infobox.setText(
        ...
        def generic_clicked(self, from_interlist: tuple):
            ...
            gen = self.crt_module.get_generic(genericname,
                                              suppress_error=True)
            if gen:
                self.crt_generic = gen
                self.crt_port = None
                self.crt_interface = None
                select_text = ("Generic '{}' ({}) with default value [{}] of "
                               "module '{}'").format(genericname, gen.data_type,
                               defaultval, self.crt_module.entity_name)
                print(str(gen))
                self.status.setText(select_text)
        ...
        def add_module(self, modname: str, reponame: str=""):
            ...
            tablerow = [mod.entity_name,
                        "AsWindowModule" if isinstance(mod, AsWindowModule)
                        else "AsModule", mod.repository_name]
        ...
        def build_interlist(self, module: AsModule):
            ...
            for inter in intercol:
                if isinstance(inter, Port):
                    port_type = "StandardPort" if \
                        isinstance(inter, StandardPort) else "Port"
                    tablerow = [inter.name, port_type, inter.direction]
                elif isinstance(inter, Generic):
                    tablerow = [inter.code_name, "Generic",
            ...
            print("Showing {} interfaces/ports/generics of module '{}'".format(
        ...
        def list_details_action(self):
            self.infobox.setText("")
            if self.view is self.PORT_VIEW:
                print("Listing details of port '{}'"
                      .format(self.crt_port.code_name))
                ...
                if self.crt_generic:
                    print("Listing details for generic '{}'"
                          .format(self.crt_generic.code_name))
                    sys.stdout = self.infobox_out_stream
                    ...
                    print("Listing details for interface '{}'"
                        .format(intername))
                    sys.stdout = self.infobox_out_stream
                    self.crt_interface.print_interface(True)
                    print("\nOf module:")
        ...
        def add_repo_action(self):
            ...
            if not folder:
                ...
                if mods:
                    ...
                    self.fit_table_to_contents(self.modlist)
                    print("Added {} modules to repository 'user'!"
                          .format(len(mods)))
                else:
        ...
        def add_repo_folder_dialog(self):
            ...
            folder_name = qtw.QFileDialog.getExistingDirectory(self,

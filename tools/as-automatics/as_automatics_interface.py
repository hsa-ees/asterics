...
## @ingroup automatics_intrep
class Interface:
    """! @brief Class representing an interface comprised of VHDL ports and
    generics.
    Interfaces have a automatically assigned name and a type.
    The type is usually based on an Interface template, a Python class
    inheriting as_automatics_interface::Interface providing a port configuration
    with core names, data types and data directions."""

    def __init__(
        self, type_name: str, template: object = None, default_name: str = ""
    ):
        self.ports = []  ## List of this interface's Ports
        self.generics = []  ## List of this interface's Generics
        self.name = default_name  ## This name
        self.type = type_name  ## Interface "type" (as_stream, AXI_Master, ...)
        ...
        ## @brief AsModule to instantiate in toplevel.
        ...
        if template is not None:
            if isinstance(template, Interface):
                ...
                LOG.error(
                    "Template passed to %s is not an Interface object!",
                    self.type,
                )

    def __str__(self) -> str:
        """! @brief Return the 'user name' of this interface (not the unique name)"""
        return self.name
    ...
    def connect(self, other):
        """! @brief Add a connection from this interface as the data source."""
    ...
    def duplicate(self):
        """! @brief Generate a copy of this interface object reseting some values.
    ...
    def make_external(self, value: bool = True):
        """! @brief Make this interface external, adding it to the IP-Core interface."""
        self.to_external = value

    ## @ingroup automatics_cds
    def instantiate_module(self, entity_name: str, in_group: str = ""):
        """! @brief Automatically add 'entity_name' to the chain
        and connect to this interface.
        Automatically add the module 'entity_name' to a module group
        (module parent by default) and connect this interface to it.
    ...
    def instantiate_no_module(self):
        """! @brief Reset the instantiation behaviour.
    ...
    def get_ports(
        ...
        """! @brief Return all ports matching 'port_name'.
        ...
        if not found and not suppress_error:
            LOG.error(
                "Could not find a port '%s' in interface '%s'!",
                port_name,
                str(self),
            )
    ...
    def get_port(self, port_name: str, *, suppress_error: bool = False) -> Port:
        """! @brief Return the first Port matching 'port_name'.
        Search for and return the first Port matching 'port_name'
        using the port.name attribute"""
        found = next(
            (port for port in self.ports if port.name == port_name), None
        )
        if not found and not suppress_error:
            LOG.error(
                "Could not find a port '%s' in interface '%s'!",
    ...
    def get_port_by_code_name(
        ...
        found = next(
            (port for port in self.ports if port.code_name == port_name), None
        )
        if not found and not suppress_error:
            LOG.error(
                "Could not find a port '%s' in interface '%s'!",
    ...
    def has_port(self, port_name: str) -> bool:
        """! @brief Return True if this interface has a Port matching 'port_name'.
    ...
    def template_has_port(self, port_name: str) -> bool:
        """! @brief Return 'True' if this interface's template has a Port matching
    ...
    def get_generics(self, generic_name: str) -> Sequence[Generic]:
        """! @brief Return all Generics matching 'generic_name'
    ...
    def get_generic(self, generic_name: str) -> Generic:
        """! @brief Returns the first generic matching 'generic_name' that's associated
        with this object.
        If none are found, returns None."""
        return next(
            (gen for gen in self.generics if gen.name == generic_name), None
        )

    def has_generic(self, generic_name: str) -> bool:
        """! @brief Return 'True' if this interface has a Generic matching
    ...
    def template_has_generic(self, generic_name: str) -> bool:
        """! @brief Return 'True' if this interface's template has a Generic matching
        'generic_name' in its generic.name attribute"""
        if self.template is not None:
    ...
    def add_generic(self, generic_obj: Generic) -> bool:
        """! @brief Add a generic to this interface instance."""
        ...
        if self.has_generic(generic_obj.name):
            LOG.debug(
                "Interface '%s' already has a generic '%s'",
                str(self),
                generic_obj.name,
            )
            return False
    ...
    def remove_generic(self, generic_name: str) -> bool:
        """! @brief Remove a generic from this interface instance
        by name 'generic_name'."""
    ...
    def get_port_name_list(self) -> Sequence[str]:
        """! @brief Return a list of the port.name attribute of all present Ports"""
        return [port.name for port in self.ports]

    def is_connect_complete(self) -> bool:
        """! @brief Returns True if all ports of this interface are connected."""
        for port in self.ports:
            if not (port.connected or port.optional):
                return False
        return True
    ...
    def set_connected_all(self, value: bool = True):
        """! @brief Set the 'connected' attribute for this interface and all Ports"""
        self.connected = value
        for port in self.ports:
            port.connected = value

    def set_prefix_suffix(self, new_prefix, new_suffix, update: bool = True):
        """! @brief Modify the prefix and suffix for all Ports.
        Does NOT store the new prefix and suffix in the respective attributes
        unless the 'update' parameter is set to True."""
        for port in self.ports:
    ...
    def fit_and_add_port(self, port_obj: Port) -> bool:
        """! @brief Adds a Port to this interface if it matches and is in the template.
        ...
        if not isinstance(port_obj, Port):
            LOG.error(
                "'fit_and_add_port' got the non-Port object '%s'.",
                repr(port_obj),
            )
            return False
        ...
        if not first_port:
            ...
            if (this_suffix != self.name_suffix) or (
                ...
                LOG.debug(
                    "Port '%s' didn't fit, prefix/suffix mismatch!",
                    port_obj.code_name,
                )
                return False
        else:
        ...
        LOG.debug(
            "Trying to add '%s' to interface '%s'",
            port_obj.code_name,
            str(self),
        )
        # Try adding the port
    ...
    def remove_port(self, port_name: str) -> bool:
        """! @brief Remove port(s) from this interface instance by name 'port_name'."""
        ...
        if len(to_remove) > 1:
            ...
            LOG.debug(
                "Removing port '%s' from interface '%s'", port_name, str(self)
            )
    ...
    def get_port_direction_normalized(self, port_name: str) -> str:
        """! @brief Return the direction of a port as it is defined in the VHDL file."""
        found = self.get_port(port_name, suppress_error=True)
        if found is None:
            LOG.error(
                "Couldn't find port '%s' in interface '%s'",
                port_name,
                str(self),
            )
            return ""
    ...
    def print_interface(self, verbose: bool = False):
        """! @brief Print the interface configuration, listing all ports and generics.
        Prints only the mandatory ports if 'verbose' is set to 'False'."""
        print(
            "{}: {} with direction '{}'".format(
                str(self), self.type, self.direction
            )
        )
        if verbose:
            print(
                "Source module: {}".format(str(as_help.get_source_module(self)))
            )
            if self.generics:
                print("\nGenerics:")
                self.list_generics()
    ...
    def list_ports(self):
        """! @brief Print a list of ports associated with this interface instance."""
        for port in self.ports:
            print(str(port))
    ...
    def clear(self):
        """! @brief Removes all ports and generics from this interface instance.
    ...
    def is_complete(self) -> bool:
        """! @brief Checks if this instance currently 'has' all the mandatory ports,
        ...
        for mtport in [
            tport for tport in self.template.ports if not tport.optional
        ]:
            ...
            if mtport.name not in present_portnames:
                LOG.debug(
                    "Port '%s' isn't present! Removing interface.", mtport.name
                )
                return False
    ...
    def get_unconnected_ports(self) -> Sequence[Port]:
        """! @brief Return a list of this interface's ports that are not connected to

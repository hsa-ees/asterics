...
class AsProcessingChain():
    ...
    def add_module(self, entity_name: str, module_name: str = "",
        ...
        LOG.debug("Chain: Adding module '%s' to processing chain as '%s' ...",
        ...
        if module_name == "":
            ...
            module_name = "{}_{}" \
                .format(entity_name, sum([1 for mod in module_list
                                          if entity_name == mod.entity_name]))
    ...
    def __group__(self, group_name: str, parent_module: AsModule,
                  *sub_modules: AsModule):
    ...
    def list_address_space(self):
        ...
        print("{} registers per ASTERICS module."
              .format(self.max_regs_per_module))
        as_conh.list_address_space(self.address_space, self.addr_per_reg,
                                   self.max_regs_per_module)
    ...
    def auto_connect(self):
        ...
        LOG.debug("Running auto_connect() for %s modules...",
                  str(len(self.modules)))
        ...
        self.max_regs_per_module = as_conh.get_max_regs_per_module(
            self.modules)
        LOG.debug("Set max_regs_per_module to '%s'.",
                  str(self.max_regs_per_module))
    ...
    def resolve_generic(port: Port) -> bool:
        ...
        for gen in port.generics:
            if gen.code_name not in gen_strs \
                    or gen.parent is not module:
    ...
    def propagate_interface_up_once(self, inter: Interface,
        ...
        ext_inter.set_prefix_suffix(
            ...
            inter.name_suffix, add_module_name)
    ...
    def handle_unconnected_ports(self, module: AsModule):
        ...
        for port in unconnected:
            LOG.debug("Handling unconnected port '%s' of '%s' ...",
            ...
            ret = self.__connect_port__(port, target,
                                        sink_parent=parent, top=self.top)
            if ret:
                if port.get_direction_normalized() == "in":
                ...
                if target:
                    LOG.debug("Port '%s' connected with '%s' of '%s'",
                              port.code_name,
                              getattr(target, "code_name", target),
                              parent.name)
                else:
                    LOG.debug("Port rules for '%s' of '%s' applied",
                              port.code_name, parent.name)
    ...
    def connect_register_interfaces(self, module: AsModule):
        ...
        for regif in module.register_ifs:
            ...
            if (self.asterics_base_addr + self.cur_addr >
                ...
                LOG.error(("Too many register interfaces or too many "
    ...
    def connect_generics(module: AsModule):
        ...
        for gen in module.generics:
            LOG.debug("Handling Generic '%s' of '%s'...",
                      gen.code_name, module.name)
            ...
            if gen.link_to and isinstance(gen.link_to, str):
                ...
                while higher_mod:
                    ...
                    fgen = higher_mod.get_generic(
                        gen.link_to, suppress_error=True)
                    if fgen:
                        LOG.debug("Found matching generic '%s' in '%s'",
                                  fgen.code_name, fgen.parent.name)
                ...
                if fgen:  # If we found a matching generic, set this
                    ...
                    for mod in module_path:
                        LOG.debug("Adding '%s' to module '%s'",
                                  gen.code_name, mod.name)
                    ...
                    LOG.warning(("Could not find the link Generic '%s'"
                                 " specified for '%s' in module '%s'!"),
                                gen.link_to, gen.code_name, module.name)
            ...
            if gen.to_external:
                ...
                for mgp in module_path:
                    if mgp.gen:  # No need to add a generic
                        LOG.info(("Generic '%s' found in module '%s' while"
                                  " making '%s' of '%s' external. Connecting"
                                  " the generics..."), mgp.gen.code_name,
                                 mgp.mod.name, gen.code_name, module.name)
                    else:  # Add a copy of the external generic
                        LOG.debug("Adding a copy of '%s' to module '%s'...",
                                  ext_name, mgp.mod.name)
    ...
    def __connect_module__(self, source: AsModule,
        ...
        LOG.debug("Looking to connect module '%s' to '%s'.", source.name,
                  str(sink))
        ret = None
        ...
        if isinstance(sink, AsModule):
            ...
            for from_inter in source.interfaces:
                ...
                for to_inter in sink.interfaces:
                    ...
                    if to_inter.direction == from_inter.direction:
                        continue
                    ret = self.__connect_interface__(from_inter, to_inter,
                                                     top=top)
                    if ret is not None:
                        break
            ...
            for from_port in source.ports:
                ...
                to_port = sink.get_port(from_port.code_name,
                                        suppress_error=True)
                # Just call the method, see what happens
                ret = self.__connect_port__(from_port, to_port,
                                            sink_parent=sink, top=top)
                if ret is not None:
                    break
    ...
    def __connect_interface__(self, source: Interface,
                              sink, *, top) -> bool:
        ...
        LOG.debug("Looking to connect interface '%s' to '%s'",
                  source.unique_name, str(sink))
        ...
        # Connect matching ports of the two Interfaces
            ...
            LOG.debug("'%s' complete: '%s'; '%s' complete: '%s'",
                      source.unique_name, str(source.connected),
                      sink.unique_name, str(sink.connected))
    ...
    def __connect_port__(self, source: Port, sink, *, sink_parent=None,
                         top):
        ...
        if not isinstance(sink, Port) and sink is not None:
            ...
            if target is not None:
                ...
                LOG.debug("Connecting '%s' to '%s'", source.code_name,
                          str(target))
    ...
    def eval_port_rules(self, source: Port, sink, sink_parent):
        ...
        for cond in source.get_rule_conditions():
            LOG.debug("Checking rule '%s' of port '%s'",
                      cond, source.code_name)
            try:
                met = Port.rule_condition_eval[cond](source, sink)
            except KeyError:
                LOG.warning("Found invalid ruleset condition '%s'. Ignored!",
                            cond)
                met = False
    ...
    def __generate_glue_signal__(port: Port, con_from: Port,
        ...
        if getattr(from_parent, "modlevel", 0) != \
                getattr(to_parent, "modlevel", 0):
            return None
        if con_from.port_type == "interface":
            signame = "{}_{}{}".format(AsModule.get_parent_module(port).name,
                                       port.parent.name_prefix, port.code_name)
        else:
            signame = "{}_{}".format(AsModule.get_parent_module(port).name,
                                     port.code_name)
        signal = GlueSignal(name=signame, data_width=port.data_width,
        ...
        while tmod is not self.top:
            ...
            if not pmod:  # Make sure tmod is not None
                LOG.error(
                    ("Make external: Error while propagating to toplevel:"
                     " Parent of Module '%s' is 'None'!"), str(tmod))
                return False
            ...
            if not new_port:
                ...
                if not pmod.add_port(new_port):
                    LOG.error(("Make external: Could not add port '%s' "
                               "to module '%s'!"), str(port), str(pmod))
                    return False
            # Connect port-pair

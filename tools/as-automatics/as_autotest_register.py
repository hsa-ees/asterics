...
class TestSlvRegIf(ut.TestCase):
    def setUp(self):
        ...
        testports = [
            Port(name="slv_status_reg", data_type="slv_reg_data", direction="out"),
            Port(name="slv_reg_modify", data_type="std_logic_vector", direction="out"),
            Port(
                name="slv_reg_config", data_type="slv_reg_config_table", direction="out"
            ),
        ]
    ...
    def test_is_complete(self):
        ...
        self.regif.ports.append(
            Port(name=tport.name, direction=tport.direction, data_type=tport.data_type)
        )
    ...
    def test_remove_port(self):
        ...
        self.assertNotIn("slv_status_reg", [port.name for port in self.regif.ports])
        ...
        self.assertNotIn("slv_reg_config", [port.name for port in self.regif.ports])
    ...
    def test_add_port_wo_pre_suffix(self):
        ...
        testport_n = Port(
            name="slv_reg_config", direction="out", data_type="slv_reg_config_table"
        )
    ...
    def test_decode_slvreg_table(self):
        ...
        testconfig_h = Constant(
            name="slave_register_configuration",
            value=('("11","11","01","01","01","10",' '"10","00","00","00");'),
        )
        result_h = [
            "HW <=> SW",
            "HW <=> SW",

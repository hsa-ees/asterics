...
class TestVHDLReader(ut.TestCase):
    ...
    def test_get_constants(self):
        ...
        self.assertEqual(
            constants[0].data_type, "slv_reg_config_table(0 to max_regs_per_module - 1)"
        )
    ...
    def test_get_generics(self):
        ...
        self.assertEqual(
            [gen.code_name for gen in generics],
            ["MAX_REGS_PER_MODULE", "IMAGENERIC", "ANOTHERONE", "LASTONE"],
        )
        self.assertEqual(
            [gen.data_type for gen in generics],
            ["data_type", "string", "expression", "intrusionattempt"],
        )
        self.assertEqual(
            [gen.default_value for gen in generics],
            [
                "genericvalue",
                '"andthisismyvalue"',
                63,
